...
```{r}
c_orig <- read.csv("~/Desktop/Current Projects/COVID/teacher_SEIR-master/Teacher Matrices R/home_orig_mx.csv", row.names=1)
...
# t/s/v/x corresponds to (teacher/staff/other)
...
for (kid in k){
  ...
  for (adult in l) {
    # portioning out among employment groups
    
    # kids-teachers
    k_l_t <- c_orig[kid, adult] * a_t[[adult-2]] 
    
    # kids-staff
    k_l_s <- c_orig[kid, adult] * a_s[[adult-2]]
    
    # kids-other
    k_l_x <- c_orig[kid, adult] * a_x[[adult-2]]
...
for (adult in l){
  ...
  for (kid in k) {
    ...
    # calc teachers-kids
    k_l_t <- c_orig[adult, kid] 
    
    # calc staff-kids
    k_l_s <- c_orig[adult, kid] 
    
    # calc other-kids
    k_l_x <- c_orig[adult, kid] 
    ...
    # employment subgroups 
     for (adult_j in l) {
      ...
      for (l_group2 in 1:3) {
        ...
        inner <- c_orig[adult_i, adult_j] * UpdatedProportions5[l_group2+5, adult_j-2] 
...
write.csv(outpt.df, "Home Contact Matrix 14x14_1.csv")
```