...
def school_close(trigger, current_date, current_infected, metro_pop):
    """
    :param trigger: str, format: type_population_number;
        example: number_all_5 or ratio_school_1 or date__20200315
    :param current_date: datetime object, current date
    :param current_infected: np array of shape (n_age, ), number of infected
    ...
    if trigger_type[0].lower() != 'd':
        ...
        # Close when reach proportion / ratio
            ...
            closure_threshold = int(trigger_value) * target_pop_size / 100.0 
...
def school_reopen(trigger, closing_infected, current_infected, closing_t,
                  current_t, current_date, interval_per_day):
    """
    :param trigger: str, format: type_population_number,
        example: monitor_all_75 (75% reduction), no_na_12 (12 weeks)
    :param closing_infected: np.array of shape (n_age, riskNum), number of cases
        on the day of closure on the day of closure - time lag?
    :param current_infected: np.array of shape (n_age, riskNum), number of
        infected
